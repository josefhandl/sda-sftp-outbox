...
import se.nbis.lega.outbox.pojo.EncryptedIntegrity;
import se.nbis.lega.outbox.pojo.FileDescriptor;
import se.nbis.lega.outbox.pojo.Operation;
...
import static se.nbis.lega.outbox.pojo.Operation.*;
...
public class OutboxSftpEventListener implements SftpEventListener {
    ...
    protected String getFilePath(Path path, String username) {
        String key = path.toFile().getAbsolutePath().replaceFirst(outboxFolder, "");
        if (key.startsWith("/")) {
            key = key.substring(1);
        }
        if (outboxFSPath != null && !outboxFSPath.trim().isEmpty()) {
            log.debug("Outbox FS path filepath is {}", outboxFSPath);
            key = outboxFSPath + "/" + key    ;
        }
    ...
    @Value("${outbox.local.s3BackendFSPath}")
    public void setOutboxFSPath(String outboxFSPath) {
        this.outboxFSPath = outboxFSPath;
    }
